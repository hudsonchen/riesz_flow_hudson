#!/bin/bash -l
#SBATCH --job-name=riesz_power1p0_top20_imagenet64
...
MASTER_PORT=${MASTER_PORT:-6673}
CONFIG=${CONFIG:-configs/gen/imagenet64_riesz_power1p0_top20.yaml}
WORKDIR=${WORKDIR:-"${RDS_ROOT}/runs/imagenet64_riesz_power1p0_top20_no_rms_lr4e4_warmup2k_ps_1"}
...
test -f riesz_power_topk.py || {
    echo "Missing powered top-k Riesz loss: ${REPO_DIR}/riesz_power_topk.py" >&2
    exit 1
}
