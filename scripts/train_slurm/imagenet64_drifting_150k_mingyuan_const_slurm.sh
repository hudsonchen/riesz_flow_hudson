#!/bin/bash -l
#SBATCH --job-name=drifting_const_imagenet64
...
CONFIG=${CONFIG:-configs/gen/imagenet64_drifting_150k_mingyuan_const.yaml}
WORKDIR=${WORKDIR:-"${RDS_ROOT}/runs/imagenet64_drifting_150k_mingyuan_const_ps_1"}

test -f "$CONFIG" || { echo "Missing config: $CONFIG" >&2; exit 1; }
test -f train.py || { echo "Missing training entrypoint: ${REPO_DIR}/train.py" >&2; exit 1; }
mkdir -p "$WORKDIR"
