#!/bin/bash -l
#SBATCH --job-name=matern32_frozen_velocity_rms_lr4e4_imagenet64
...
CONFIG=${CONFIG:-configs/gen/imagenet64_matern32_rms_lr4e4.yaml}
WORKDIR=${WORKDIR:-"${RDS_ROOT}/runs/imagenet64_matern32_frozen_velocity_rms_multiscale_lr4e4_ps_1"}
...
test -f matern_rms.py || {
    echo "Missing RMS Matern-3/2 loss: ${REPO_DIR}/matern_rms.py" >&2
    exit 1
}
