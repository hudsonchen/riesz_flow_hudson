...
def _apply_riesz_power(
    ...
    """
    Convert Euclidean distances d into Riesz distances d**r.

    For r < 1, the derivative of d**r is singular at d = 0. We clamp
    distances before applying the power, then restore exact diagonal
    zeros for same-set distance matrices.
    """
    powered = distance.clamp_min(epsilon).pow(riesz_power)

    if same_set:
        n_left = powered.shape[-2]
        n_right = powered.shape[-1]

        if n_left != n_right:
            raise ValueError(
                "same_set=True requires a square pairwise distance matrix"
            )

        diagonal_mask = torch.eye(
            n_left,
            device=powered.device,
            dtype=torch.bool,
        )

        while diagonal_mask.ndim < powered.ndim:
            diagonal_mask = diagonal_mask.unsqueeze(0)

        powered = powered.masked_fill(diagonal_mask, 0.0)
...
def _full_riesz_terms(
    ...
    """
    distance_gen_pos = _apply_riesz_power(
    ...
    distance_gen_gen = _apply_riesz_power(
    ...
    distance_pos_pos = _apply_riesz_power(
    ...
    if neg_scaled.shape[1] > 0:
        distance_gen_neg = _apply_riesz_power(
...
def _sliced_riesz_terms(
    ...
    # Move the projection dimension before the particle dimensions so
    # the same-set diagonal mask applies to the two particle axes.
...
def riesz_loss(
    ...
    for every projection:
        ...
        2 E|<theta,G>-<theta,P>|^r
        - E|<theta,G>-<theta,G'>|^r
        - E|<theta,P>-<theta,P'>|^r
        - 2 E|<theta,G>-<theta,U>|^r.
    ...
    if not 0.0 < riesz_power < 2.0:
        raise ValueError(
            "riesz_power must satisfy 0 < riesz_power < 2, "
            f"but received {riesz_power}"
        )
