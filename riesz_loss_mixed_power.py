...
def _mixed_riesz_distance(
    distance: torch.Tensor,
    riesz_powers: Sequence[float],
    epsilon: float,
    reference_distance: float,
) -> torch.Tensor:
    """
    Apply a fixed mixture of Riesz powers to a distance matrix.

    The normalization around reference_distance keeps the powers on a
    comparable numerical scale. With riesz_powers=(1.0,), this returns
    the original distance exactly.
    """
    zero_mask = distance == 0
    normalized = (distance / float(reference_distance)).clamp_min(epsilon)

    mixed = torch.stack(
        [normalized.pow(float(power)) for power in riesz_powers],
        dim=0,
    ).mean(dim=0)

    mixed = mixed * float(reference_distance)
    return mixed.masked_fill(zero_mask, 0.0)
...
def _full_riesz_terms(
    ...
    """
    reference_distance = float(gen_scaled.shape[-1] ** 0.5)

    distance_gen_pos = _mixed_riesz_distance(
        torch.cdist(gen_scaled, pos_scaled),
        riesz_powers,
        epsilon,
        reference_distance,
    )
    distance_gen_gen = _mixed_riesz_distance(
        torch.cdist(gen_scaled, gen_scaled),
        riesz_powers,
        epsilon,
        reference_distance,
    )
    distance_pos_pos = _mixed_riesz_distance(
        torch.cdist(pos_scaled, pos_scaled),
        riesz_powers,
        epsilon,
        reference_distance,
    )
    ...
    if neg_scaled.shape[1] > 0:
        distance_gen_neg = _mixed_riesz_distance(
            torch.cdist(gen_scaled, neg_scaled),
            riesz_powers,
            epsilon,
            reference_distance,
        )
...
def riesz_loss(
    ...
    riesz_powers: Sequence[float] = (1.0,),
    ...
    riesz_powers = tuple(float(power) for power in riesz_powers)
    if not riesz_powers:
        raise ValueError("riesz_powers must contain at least one value")
    if any(not 0.0 < power < 2.0 for power in riesz_powers):
        raise ValueError("Every Riesz power must satisfy 0 < r < 2")
    ...
    if use_sliced and riesz_powers != (1.0,):
        raise ValueError(
            "Mixed powers are implemented only for full Riesz. "
            "Set use_sliced=False."
        )
    ...
    info = {
        ...
        "riesz_power_min": torch.tensor(min(riesz_powers), device=loss.device),
        "riesz_power_max": torch.tensor(max(riesz_powers), device=loss.device),
        "riesz_power_count": torch.tensor(
            float(len(riesz_powers)),
            device=loss.device,
        ),
    }
